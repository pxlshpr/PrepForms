...
extension GoalModel {
    var copy: GoalModel {
        GoalModel(
            id: self.id,
            goalSet: self.goalSetModel,
            goalSetType: self.goalSetType,
            type: self.type,
            lowerBound: self.lowerBound,
            upperBound: self.upperBound,
            isAutoGenerated: self.isAutoGenerated
        )
    }
}
...
//        }
    ...
    func lowerBoundSuffix(equivalent: Bool) -> String? {
        guard !(usesSingleValue && equivalent) else { return nil }
        guard !hasValidUpperBound else { return nil }
        return equivalent ? equivalentUnitString : type.unitString
    }

    func upperBoundSuffix(equivalent: Bool) -> String? {
        equivalent ? equivalentUnitString : type.unitString
    }

    func lowerBoundPrefix(equivalent: Bool) -> String? {
        guard !(usesSingleValue && equivalent) else {
            return nil
        }
        guard hasValidLowerBound, !hasValidUpperBound else { return nil }
        if equivalent {
            return "at least"
        } else {
            return usesSingleValue ? "within" : "at least"
        }
    }
    
    func upperBoundPrefix(equivalent: Bool) -> String? {
        guard !(usesSingleValue && equivalent) else {
            return "to"
        }
        guard hasValidUpperBound else { return nil }
        return hasValidLowerBound ? "to" : "at most"
    }
...
extension GoalModel {
    ...
    func validateLowerBoundLowerThanUpper() {
        ...
        var boundsInWrongOrder: Bool {
            if self.type.energyDelta == .deficit {
                /// Only when specifying deficits, the lower bound should be *higher* than the upper bound
                /// (so that the equivalent values—which are presented in increasing magnitue—coincide with
...
extension GoalModel {
    func hasSameContentsAs(_ other: GoalModel) -> Bool {
        self.goalSetType == other.goalSetType
        && self.type == other.type
        && self.lowerBound == other.lowerBound
        && self.upperBound == other.upperBound
        && self.isAutoGenerated == other.isAutoGenerated
    }
}
